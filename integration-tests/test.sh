...
function run_hurl() {
    local hurl_args=$1
    # --userns=keep-id
    podman run --rm -v $PWD/hurl:/hurl:z,ro --workdir /hurl --net=host ghcr.io/orange-opensource/hurl:latest --file-root /hurl --variables-file /hurl/variables.env --jobs 1 --test ${hurl_args}
}
